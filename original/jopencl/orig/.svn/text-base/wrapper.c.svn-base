...
void toclImFormat(JNIEnv *jenv, jobject jobj, cl_image_format *format)
	{
	GETCLASS
	jfieldID fidOrder=(*jenv)->GetFieldID(jenv, jobj, "image_channel_order",  "I");
	jfieldID fidType=(*jenv)->GetFieldID(jenv, jobj, "image_channel_data_type",  "I");
	format->image_channel_order=(*jenv)->GetIntField(jenv, jobj, fidOrder);
	format->image_channel_data_type=(*jenv)->GetIntField(jenv, jobj, fidType);
	}
...
JNIEXPORT jint JNICALL Java_javax_opencl_CLMem__1createImage2Dbyte
  (JNIEnv *jenv, jobject jobj, jint cid, jint memFlags, jobject imageFormat, jint imageW, jint imageH, jint imageRowPitch, jbyteArray initData)
	...
	cl_int ret;
	jsize alen = (*jenv)->GetArrayLength(jenv, initData);
	jbyte *abody = (*jenv)->GetByteArrayElements(jenv, initData, 0);
	...
	(*jenv)->ReleaseByteArrayElements(jenv, initData, abody, 0);
	SETID(mid)
	return ret;
	}
